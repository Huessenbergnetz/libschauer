...
class SCHAUER_LIBRARY SJob : public QObject
{
     ...
     *
     * \par Notifier signal
     * \li void result(SJob *job)
     *
     * \sa SJob::errorString, SJob::errorText
     */
     ...
     * associated with the \link SJob::error error\endlink, such as a URL.
     *
     * \par Access functions
     ...
     * \li void result(SJob *job)
     *
     * \sa SJob::error, SJob::errorString
     */
     ...
     *
     * \par Notifier signal
     * \li void percent(SJob *job, unsigned long percent)
     */
     ...
     */
    Q_PROPERTY(Schauer::SJob::Capabilities capabilities READ capabilities CONSTANT)
public:
    /*!
     * \brief Error codes.
     */
    ...
    /*!
     * \brief Constructs a new %SJob object with the given \a parent.
     */
    explicit SJob(QObject *parent = nullptr);

    /*!
     * \brief Desconstructs the %SJob obejct.
     */
    ~SJob() override;
     ...
     * use the asynchronous interface of SJob instead.
     *
...
Q_SIGNALS:
     ...
     *
     * \note This is a private signal, it can’t be emitted directly by subclasses of %SJob, use emitResult() instead.
     *
     ...
     *
     * \note This is a private signal, it can’t be emitted directly by subclasses of %SJob.
     *
    ...
    /*!
     * \brief Emitted when the job is finished (except when killed with SJob::Quietly).
     *
     ...
     */
    void description(Schauer::SJob *job, const QString &title,
...
protected:
     ...
     *
     * You should define an (anonymous) enum of error codes, with values starting at SJob::UserDefinedError,
...
Q_DECLARE_METATYPE(Schauer::SJob::Unit)
Q_DECLARE_OPERATORS_FOR_FLAGS(Schauer::SJob::Capabilities)

#endif // SCHAUER_SJOB_H
